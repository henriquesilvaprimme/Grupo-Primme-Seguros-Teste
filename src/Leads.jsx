...
const Leads = ({ leads, usuarios, onUpdateStatus, transferirLead, usuarioLogado, fetchLeadsFromSheet }) => {
  ...
  // NOVO: Adicionado estado para controlar se uma edição está em andamento.
  ...
  const handleRefreshLeads = async () => {
    // Adicionado: Não executa o refresh se alguma edição estiver em andamento.
    const isAnyLeadBeingEdited = Object.values(isEditing).some(status => status);
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <button
            ...
            disabled={isLoading || Object.values(isEditing).some(status => status)}
            style={{
              background: 'none',
              border: 'none',
              cursor: (isLoading || Object.values(isEditing).some(status => status)) ? 'not-allowed' : 'pointer',
              padding: '0',
              display: 'flex',
              alignItems: 'center',
