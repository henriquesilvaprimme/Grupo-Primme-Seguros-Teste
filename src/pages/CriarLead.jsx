...
const CriarLead = ({ adicionarLead }) => {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto bg-white rounded-xl shadow-md space-y-6">
      ...
      <div className="flex flex-col items-center"> {/* Alterado para centralizar o botão */}
        <button
          onClick={handleCriar}
          className="bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600 transition"
        >
          Criar Lead {/* Texto do botão alterado */}
        </button>
        {mensagemSucesso && (
          <p className="mt-4 text-green-600 font-semibold text-center">{mensagemSucesso}</p> // Mensagem de sucesso abaixo do botão
        )}
      </div>
    </div>
  );
