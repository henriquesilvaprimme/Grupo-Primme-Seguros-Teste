...
const LeadsFechados = ({ leads, usuarios, onUpdateInsurer, onConfirmInsurer, onUpdateDetalhes, fetchLeadsFechadosFromSheet, isAdmin }) => {
  ...
  // Hook para a rolagem automática
  useEffect(() => {
    let scrollInterval;
    if (fechadosFiltradosInterno.length > 0) {
      scrollInterval = setInterval(() => {
        window.scrollBy({
          top: window.innerHeight * 0.8, // Rola 80% da altura da janela
          behavior: 'smooth',
        });
        // Reinicia a rolagem para o topo quando chega ao fim da página
        if (window.innerHeight + window.scrollY >= document.body.offsetHeight) {
          window.scrollTo({ top: 0, behavior: 'smooth' });
        }
      }, 5000); // Rola a cada 5 segundos
    }
    return () => clearInterval(scrollInterval); // Limpa o intervalo ao desmontar ou atualizar
  }, [fechadosFiltradosInterno]);
